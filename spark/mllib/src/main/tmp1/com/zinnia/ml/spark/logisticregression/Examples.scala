package com.zinnia.ml.spark.logisticregression
...
object Ex2_Data2_Poly{
  def main(args: Array[String]) {
    ...
    println("Number of features before polynomial is: "+labelledRDD.first().features.toArray.length)
    val degreeOfPoly = 4
    ...
    println("Number of features after polynomial is "+labelledRDD.first().features.toArray.length)
    println("Normalizing features")
    val featureScaledData = regression.normaliseFeatures(labelledRDD)
    labelledRDD = featureScaledData._1.cache()
    val featureMean = featureScaledData._2
    val featureStd = featureScaledData._3
    val thetaArray = new Array[Double](labelledRDD.first().features.toArray.length)
    println("Running Regression")

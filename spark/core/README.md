...
This program must be packaged, as a jar containing this code and any depen-
...
replicate/spark/variable-scaling). Two classes in the package (be.ing.apa.spark.scaling)
have a main method:
...
--class "be.ing.apa.spark.scaling.EvaluateAndApply"\
...
ing set.
$APA_DATA/donut/scaled is the target directory to save the scaled training
set, in a file named part-00000.
$APA_DATA/donut/scale.csv is the file where the scaling parameters (feature,
min, max) are saved:
...
$ $SPARK_HOME/bin/spark-submit\
--class "be.ing.apa.spark.scaling.Apply"\
--master spark://$SPARK_PUBLIC_DNS:7077 $SCALING_JAR \
...
set.
$APA_DATA/donut/scale.csv is the previously computed scaling parameters
(feature, min, max)
...
$APA_REPO/algorithm/variable-scaling/spark/variable-scaling.
Usage is as follows:
18cd $APA_REPO/algorithm/variable-scaling/spark/variable-scaling
SAMPLING_JAR=‘pwd‘/target/scala-2.10/case-control-resampling_2.10-0.1-SNAPSHOT.jar
SPARK_PUBLIC_DNS=‘hostname‘
$SPARK_HOME/bin/spark-submit --class "be.ing.apa.spark.resampling.Main" \
...
231532
saving data in $APA_DATA/train-scaled-resampled
...
5.9
R interface
SparkR is an interface for R exposing the RDD API of Spark. This provides
...
to do th following in our R console:
> ### THe SPARK MASTER
...
[SparkR] Initializing with classpath /home/xavier/packages/SparkR-pkg/lib/SparkR/sparkr-as

...
SELECT EVENT.DIFFUSION_ID, 
DIFFUSION.MEMBER_ID, 
EVENT.EVENT_DATE,
DIFFUSION.NAME,EVENT.SID   
FROM DIFFUSION, EVENT  
WHERE ITEM_BUNDLE_SID 
IN (SELECT SID FROM ITEM_BUNDLE WHERE MEMBER_ID = 2 )AND EVENT_TYPE_ID = 6
...
SELECT *
FROM  ITEM_DIFFUSED 
WHERE  DIFFUSED_TYPE_SID = (SELECT SID FROM ITEM_DIFFUSED_TYPE WHERE  NAME= 'MAIL')  ;
